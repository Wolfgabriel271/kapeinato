...
session_start();
// Description: Starts the PHP session to manage user authentication state.
// Function: Initializes or resumes a session for storing admin login information.
// Technical: Must be called before any output; uses cookies or URL parameters to track sessions.

include 'db.php';
// Description: Includes the database configuration file.
// Function: Establishes database connection and makes it available as $conn.
// Technical: Requires db.php to be in the same directory; $conn is a global MySQLi connection object.
...
if (isset($_GET['delete_item'])) {
    ...
    if ($del_result && $del_result['image_path'] !== 'default.jpg') {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_item'])) {
    ...
    if (empty($name) || $price <= 0 || $edit_id <= 0) {
        ...
        $img_stmt->close();
        $image_path = $img_result['image_path'] ?? 'default.jpg';
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($mime, $allowed_types)) {
                ...
                if ($image_path !== 'default.jpg' && file_exists("uploads/" . $image_path)) {
...
// ---- Handle Online Order Status Updates ----
...
if (isset($_GET['online_order_id']) && isset($_GET['status'])) {
    ...
    if (in_array($oo_status, $allowed)) {
        ...
        // Send email notification (simulated log for demo)
        $order_info = $conn->query("SELECT email, customer_name FROM online_orders WHERE id = $oo_id")->fetch_assoc();
...
<head>
    ...
    <style>
        ...
        .analytics-card { background: rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; }
        .search-box { width: 100%; padding: 12px 16px; border: 1px solid var(--border-subtle); border-radius: 8px; background: rgba(0,0,0,0.3); color: var(--text-primary); font-size: 1rem; margin-bottom: 20px; }
        .search-box:focus { outline: none; border-color: var(--amber); }
        .inventory-status { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
        .in-stock { background: rgba(34,197,94,0.2); color: #22c55e; }
        .low-stock { background: rgba(234,179,8,0.2); color: #eab308; }
        .out-of-stock { background: rgba(239,68,68,0.2); color: #ef4444; }
        .edit-form-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 1000; justify-content: center; align-items: center; }
        .edit-form-container { background: var(--bg-void); border-radius: 16px; padding: 30px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto; border: 1px solid var(--border-subtle); }
        .rating-stars { color: #fbbf24; font-size: 1.2rem; }
        .chat-widget { position: fixed; bottom: 20px; right: 20px; width: 350px; height: 450px; background: white; border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); display: none; flex-direction: column; z-index: 1000; overflow: hidden; }
        .chat-header { background: linear-gradient(135deg, #065f46, #047857); color: white; padding: 16px; }
        .chat-messages { flex: 1; overflow-y: auto; padding: 16px; }
        .chat-input { border-top: 1px solid #e5e7eb; padding: 12px; display: flex; gap: 8px; }
        .chat-input input { flex: 1; padding: 10px; border: 1px solid #d1d5db; border-radius: 20px; }
        .chat-toggle { position: fixed; bottom: 20px; right: 20px; width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #065f46, #047857); color: white; border: none; cursor: pointer; font-size: 24px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); z-index: 999; }
    </style>
</head>
<body>

<nav>
    <div class="nav-logo">Kape En Ato</div>
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        ...
        <div class="admin-panel">
            ...
            <form method="POST" action="admin.php?tab=menu" enctype="multipart/form-data" class="add-item-form">
                ...
                <div class="form-group">
                    ...
                    <?php 
                    $current_img = "uploads/" . $edit_item['image_path'];
                    if (file_exists($current_img) && $edit_item['image_path'] !== 'default.jpg'): 
                    ?>
                        <img src="<?= $current_img ?>" alt="Current" style="max-width:200px; border-radius:8px; margin-bottom:10px;">
                    <?php else: ?>
                        <p style="color:var(--text-muted);">Using default placeholder image</p>
                    <?php endif; ?>
        ...
        <div class="admin-panel">
            ...
            <!-- Per-customer cards -->
            <?php $cust_index = 0; foreach ($customers_orders as $ckey => $cdata): $cust_index++; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php $ai = 0; foreach ($customers_orders as $ck => $cd): $ai++; if ($cd['pending_count'] > 0): ?>
